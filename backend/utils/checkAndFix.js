...
const checkAndFix = async () => {
  try {
    ...
    const admin = await pool.query('SELECT * FROM users WHERE email = $1', ['admin@assetflow.bg']);
    ...
    // Test login response
    const loginTest = await pool.query('SELECT id, username, email, role, is_system_admin FROM users WHERE email = $1', ['admin@assetflow.bg']);
